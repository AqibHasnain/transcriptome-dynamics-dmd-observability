...
def process_df(df,sampleLabels,nreps,reps,ntimepts,txIDs):
	...
	data_c_orig, data_t_orig = smooth_time_series(data_c_orig[:,:-1,reps]), smooth_time_series(data_t_orig[:,:-1,reps]) # note we are not using the final timepoint due to experimental issues
	...
	data_c, data_t = data_c_orig[:,2:]	, data_t_orig[:,2:]
